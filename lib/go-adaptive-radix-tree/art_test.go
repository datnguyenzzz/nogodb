...
func Test_art_str_InsertAndRemoveNode(t *testing.T) {
	type param struct {
		desc             string
		...
		t.Run(tc.desc, func(t *testing.T) {
			...
			for _, action := range tc.actions {
				if action.Kind == internal.InsertAction {
					...
					assert.NoError(t, err, "shouldn't fail to insert new key")
				} else {
					_, err := art.Delete(ctx, action.Key)
					assert.NoError(t, err, "shouldn't fail to delete new key")
				}
			}
