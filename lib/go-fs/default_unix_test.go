...
func TestLock(t *testing.T) {
	...
	if child {
		...
		f, err := os.CreateTemp(os.TempDir(), "golang-nogodb-testlock-")
	...
	defaultUnix := NewDefaultUnix()
	lock, err := defaultUnix.Lock(filename)
	if err != nil {
		t.Fatalf("Could not lock %s: %v", filename, err)
	}

	if !child {
		t.Logf("Spawning child process, should fail to grab lock.")
...
func TestLockSameProcess(t *testing.T) {
	f, err := os.CreateTemp(os.TempDir(), "nogodb-testlocksameprocess-")
	require.NoError(t, err)
	filename := f.Name()
	...
	defer os.Remove(filename)
	defaultUnix := NewDefaultUnix()
	lock1, err := defaultUnix.Lock(filename)
	require.NoError(t, err)
	...
	// current process.
	_, err = defaultUnix.Lock(filename)
	require.Error(t, err)
