...
func (w *WalSuite) Test_Iterator_Seeking_Ops_single_table_MVCC_col_block() {
	type param struct {
		name                string
		isUnique            bool
		cacheSize           int // 0 means no cache
...
func (w *WalSuite) Test_Iterator_Concurrently_Seeking_Ops_multiple_tables_MVCC_col_block() {
	type param struct {
		name                string
		restart             int
		isUnique            bool
...
func (w *WalSuite) Test_Iterator_First_Then_Next_Ops_MVCC_colblock() {
	type param struct {
		name      string
		restart   int
		isUnique  bool
